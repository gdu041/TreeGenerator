...
(function (window) {
	...
	var draw = function (vec) {
		var a = new THREE.Mesh(new THREE.SphereGeometry(0.01, 32), new THREE.MeshPhongMaterial({ color: 0xff0000, side: THREE.DoubleSide, depthTest: false }));
		a.position.x = vec[0];
		a.position.y = vec[1] - 2.8;
		a.position.z = vec[2];
		scene.add(a);
	}
